...
import { useCreateAmenity, useSetAmenityImages } from '@/features/amenities/api';
...
export default function AddAmenityScreen() {
  ...
  async function handleSubmit(value: AmenityFormValue) {
    ...
    try {
      ...
      const amenity = await createAmenity.mutateAsync({ societyId: profileQuery.data.society_id, ...details });
      if (photos.length > 0) {
        await setAmenityImages.mutateAsync({ amenityId: amenity.id, societyId: amenity.society_id, photos, previousPaths: [] });
      }
  ...
  return <AmenityForm title="Add amenity" submitLabel="Add amenity" isPending={createAmenity.isPending || setAmenityImages.isPending} onSubmit={handleSubmit} />;
}
