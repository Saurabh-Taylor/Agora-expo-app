...
import { getErrorMessage, refetchQueries } from '@/commonFunctions';
...
export default function ResidentVehiclesScreen() {
  ...
  const assignedSlots = (layoutQuery.data ?? []).filter(
    (slot) => slot.assignment?.flat_id === profile?.flat_id,
  );
  const isLoading = profileQuery.isLoading || vehiclesQuery.isLoading || layoutQuery.isLoading;
  const isError = profileQuery.isError || vehiclesQuery.isError || layoutQuery.isError;
  const isRetrying =
    profileQuery.isRefetching || vehiclesQuery.isRefetching || layoutQuery.isRefetching;
  const hasResidentContext = !!profile?.society_id && !!profile.flat_id;
  const isMutating =
    createVehicle.isPending || updateVehicle.isPending || deactivateVehicle.isPending;
  const isSaveDisabled = isMutating || (!editingVehicle && !registration.trim());
  ...
  async function saveVehicle() {
    if (!profile?.society_id || isSaveDisabled) return;
  ...
  async function removeVehicle(vehicle: ResidentVehicle) {
    if (!profile?.society_id || isMutating) return;

    try {
      await deactivateVehicle.mutateAsync({
        societyId: profile.society_id,
    ...
    <ScrollView
      ...
      <View style={styles.headerRow}>
        ...
        <View style={styles.flex}>
          ...
          <Text style={styles.subtitle}>Vehicles and parking assigned to your flat</Text>
        </View>
      </View>
      ...
      <AsyncState
        ...
        onRetry={() => refetchQueries(profileQuery, vehiclesQuery, layoutQuery)}
        isEmpty={!isLoading && !isError && !hasResidentContext}
        emptyTitle="No flat assigned"
        emptyMessage="Ask your society admin to assign your resident account to a flat."
      />

      {!isLoading && !isError && hasResidentContext && (
        <>
          <Text style={styles.sectionTitle}>Assigned parking</Text>
          {assignedSlots.length > 0 ? (
            <ParkingLayoutGrid slots={assignedSlots} />
          ) : (
            <EmptyCard
              title="No parking assigned"
              message="Your society admin can assign a slot after you add a vehicle."
            />
          )}
          ...
          {vehicles.length > 0 ? (
            <View style={styles.list}>
              {vehicles.map((vehicle) => (
                <View key={vehicle.id} style={styles.vehicleCard}>
                  <View style={styles.flex}>
                    <Text style={styles.registration}>{vehicle.registration_number}</Text>
            ...
            <EmptyCard
              ...
              message="Add your car, bike, EV, or other vehicle below."
            />
          )}
          ...
          <View style={styles.formCard}>
            ...
            <TextInput
              editable={!editingVehicle}
              value={registration}
              onChangeText={setRegistration}
              autoCapitalize="characters"
              placeholder="MH12AB1234"
              placeholderTextColor={Colors.textFaint}
              style={[styles.input, !!editingVehicle && styles.readonly]}
              accessibilityLabel="Vehicle registration number"
            />

            <Text style={styles.label}>TYPE</Text>
            <View style={styles.choices}>
              {ResidentVehicleTypes.map((type) => {
            ...
            <TextInput
              ...
              placeholderTextColor={Colors.textFaint}
              style={styles.input}
              accessibilityLabel="Vehicle make and model"
            />
            ...
            <Pressable
              accessibilityRole="button"
              accessibilityState={{ disabled: isSaveDisabled, busy: isMutating }}
              disabled={isSaveDisabled}
              onPress={saveVehicle}
              style={[styles.saveButton, isSaveDisabled && styles.disabled]}>
...
const styles = StyleSheet.create({
  ...
  emptyTitle: { fontWeight: '700', fontSize: 14.5, color: Colors.textPrimary },
  emptyText: { marginTop: 4, fontSize: 12.5, lineHeight: 18, color: Colors.textMuted },
  list: { gap: 9 },
  vehicleCard: {
    minHeight: 76,
    padding: 13,
  ...
  registration: { fontWeight: '800', fontSize: 15, color: Colors.textPrimary },
  vehicleMeta: { marginTop: 3, fontSize: 12, color: Colors.textMuted },
  smallButton: {
    minHeight: 44,
  ...
  smallButtonLabel: { fontWeight: '700', fontSize: 12, color: Colors.success700 },
  removeButton: { backgroundColor: '#FCEBE8' },
  removeLabel: { fontWeight: '700', fontSize: 12, color: Colors.danger700 },
  formCard: {
    marginTop: 22,
    padding: 16,
    borderRadius: Radius.card,
  ...
  readonly: { backgroundColor: '#EEECE7', color: Colors.textMuted },
  ...
  choiceLabel: { fontWeight: '700', color: Colors.textPrimary },
  ...
  saveLabel: { fontSize: 14.5, fontWeight: '700', color: Colors.textOnDark },
