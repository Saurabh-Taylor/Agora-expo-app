...
select lives_ok($$select public.create_admin_amenity('  Gym  ','Fitness','06:00','22:00','EXCLUSIVE',120,1,7,1,true,'Wear suitable shoes')$$,'admin creates amenity');
reset role;
...
select throws_ok($$select public.update_admin_amenity('57000000-0000-0000-0000-000000000001','Cross',null,'07:00','21:00','EXCLUSIVE',120,1,7,1,true,null)$$,'42501','Amenity is not available to this admin','cross-society update denied');
select set_config('request.jwt.claim.sub','47000000-0000-0000-0000-000000000001',true);
...
select set_config('request.jwt.claim.sub','47000000-0000-0000-0000-000000000003',true);
select lives_ok($$select public.create_resident_amenity_booking((select id from public.amenities where name='Gym'),(select id from public.amenity_slots where amenity_id=(select id from public.amenities where name='Gym') and start_time='10:00'),(now() at time zone 'Asia/Kolkata')::date+2)$$,'resident creates booking');
reset role;
...
select throws_ok($$select public.create_resident_amenity_booking((select id from public.amenities where name='Gym'),(select id from public.amenity_slots where amenity_id=(select id from public.amenities where name='Gym') and start_time='10:00'),(now() at time zone 'Asia/Kolkata')::date+2)$$,'23P01','Booking slot is no longer available','capacity rejected');
select set_config('request.jwt.claim.sub','47000000-0000-0000-0000-000000000003',true);
select is((select count(*)::int from public.get_amenity_slot_availability((select id from public.amenities where name='Gym'),(now() at time zone 'Asia/Kolkata')::date+2)),8,'availability returns generated slots');
select set_config('request.jwt.claim.sub','47000000-0000-0000-0000-000000000002',true);
