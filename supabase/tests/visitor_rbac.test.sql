...
set local role authenticated;
select set_config('request.jwt.claim.sub', '40000000-0000-0000-0000-000000000001', true);
select throws_ok($$ update public.profiles set role = 'GUARD', flat_id = null where id = '40000000-0000-0000-0000-000000000001' $$, '42501', 'Profile authorization fields cannot be changed by this account', 'resident cannot escalate profile role');

select throws_like($$ insert into public.visitor_requests (id, society_id, visitor_id, flat_id, status, is_pre_approved, decision_by, decision_at, gate_pass_code) values ('60000000-0000-0000-0000-000000000007', '10000000-0000-0000-0000-000000000001', '50000000-0000-0000-0000-000000000001', '30000000-0000-0000-0000-000000000002', 'APPROVED', true, '40000000-0000-0000-0000-000000000001', now(), '111 222') $$, '%row-level security%', 'resident cannot pre-approve another flat');
select throws_like($$ insert into public.visitor_requests (id, society_id, visitor_id, flat_id, status, is_pre_approved, decision_by, decision_at, gate_pass_code) values ('60000000-0000-0000-0000-000000000008', '10000000-0000-0000-0000-000000000001', '50000000-0000-0000-0000-000000000001', '30000000-0000-0000-0000-000000000001', 'EXITED', true, '40000000-0000-0000-0000-000000000001', now(), '111 222') $$, '%row-level security%', 'resident cannot pre-approve directly into an invalid state');
select lives_ok($$ insert into public.visitor_requests (id, society_id, visitor_id, flat_id, status, is_pre_approved, decision_by, decision_at, gate_pass_code) values ('60000000-0000-0000-0000-000000000009', '10000000-0000-0000-0000-000000000001', '50000000-0000-0000-0000-000000000001', '30000000-0000-0000-0000-000000000001', 'APPROVED', true, '40000000-0000-0000-0000-000000000001', now(), '111 222') $$, 'resident can create a valid own-flat pre-approval');
select set_config('request.jwt.claim.sub', '40000000-0000-0000-0000-000000000003', true);
select throws_like($$ insert into public.visitor_requests (id, society_id, visitor_id, flat_id, raised_by, status) values ('60000000-0000-0000-0000-000000000010', '10000000-0000-0000-0000-000000000001', '50000000-0000-0000-0000-000000000001', '30000000-0000-0000-0000-000000000001', '40000000-0000-0000-0000-000000000003', 'APPROVED') $$, '%row-level security%', 'guard cannot create a request already approved');
select lives_ok($$ insert into public.visitor_requests (id, society_id, visitor_id, flat_id, raised_by, status) values ('60000000-0000-0000-0000-000000000011', '10000000-0000-0000-0000-000000000001', '50000000-0000-0000-0000-000000000001', '30000000-0000-0000-0000-000000000001', '40000000-0000-0000-0000-000000000003', 'PENDING') $$, 'guard can create a clean pending request');
