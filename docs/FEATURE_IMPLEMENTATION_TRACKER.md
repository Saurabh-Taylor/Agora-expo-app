...
| SUB-01 | README setup and demo credentials | Demonstrable account required | Demonstrable account required | Demonstrable account required | Validate | README includes setup, environment variables, and non-production demo credentials | Verify hosted users have matching active profiles, society assignment, and resident flat assignment |
| SUB-02 | Installable build/APK | Required journey | Required journey | Required journey | Validate | Android Expo export passes | Produce and physically test an authenticated EAS preview APK |
| SUB-03 | Screenshots and demo video | Show resident journey | Show guard journey | Show admin journey | Validate | Screenshots and the hero workflow MP4 are committed under `docs/` | Re-capture or confirm final three-role journeys on the release build |
...
| 2026-08-08 | Audited the account-lifecycle hardening branch, aligned all Expo SDK 57 patch dependencies, refreshed the current verification count, and reconciled submission status with the committed README, screenshots, and demo video. | TypeScript, ESLint, Edge Function bundle, Edge Function unit tests, Expo Doctor 20/20, 512 pgTAP assertions, DB lint, migration list, and Android export |
