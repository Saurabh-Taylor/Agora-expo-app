...
| --- | --- | --- | --- | --- | --- | --- | --- |
| AUTH-01 | Supabase authentication and persisted session | Sign in and recover own account | Sign in and recover own account | Sign in and recover own account | Complete | Auth routes, protected persistence, profile query, root routing shell | Keep demo credentials out of production config |
...
| VIS-06 | Visitor history | Own flat only | Own society movement/history | Read-only own-society history | Complete | Resident gate tab, guard movement log, paginated admin history, indexed society/RLS query, and pgTAP tests | Final physical-device visual check |
...
| VIS-07 | Optional visitor metadata | May provide purpose, vehicle, photo, and richer pre-approval details | May capture/verify provided metadata | Read-only history only | Optional | Explicitly classified optional on July 21, 2026 | Implement only after required scope and release gates |
